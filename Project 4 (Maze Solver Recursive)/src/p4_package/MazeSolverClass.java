/**
 * MazeSolverClass is used to solve mazes 1-11 that were given to us.
 * CS 249 
 * DUE: 2-19-19
...
public class MazeSolverClass
{
	 ...
	 * @param
	 */
	public MazeSolverClass(int rowHeight, int colWidth, int inStartY,
						   int inStartX, char[][] inMaze) 
	{
		...
		maze = new char [ rowHeight ][ colWidth ];
		// for loop that goes through maze rows and columns
		for( rowIndex = 0; rowIndex < rowHeight; rowIndex++ ) 
		{
			for( colIndex = 0; colIndex < colWidth; colIndex++ )
			{
	...
	/**
     * Method shows current state of array if diagnostic run flag is set;
     * method also pauses program run for time duration previously
     * specified in setDiagnosticDisplay
     * <p>
    ...
    /**
     * Uses x and y starting position to find starting direction of maze mover
     * 
    ...
    /**
     * Method looks from center of given cell in the current direction to see
     * if the maze mover is in a perimeter cell pointing at an
    ...
    /**
     * Method tests current specified location to see if it is one of the
     * three possible wall characters
     * <p>
     * Note: Method does not use if/else or switch
     * 
     ...
     * @return Boolean result of specified test
     */
    private boolean isWallCharacter(int yLoc,
                                    int xLoc)
    {
    ...
    /**
     * Method looks from center of given cell in the current direction to
     * see if the maze mover is in a cell pointing at a wall
     * <p>
     * Note: Method does not use if/else or switch
     * 
     * @param currentLoc LocationData object holding the maze mover's 
     ...
     * current location status
     * 
     * @param marker character value holding marker to use for showing
     * maze mover trail
     * 
     * @return LocationData result of moving to next cell
     */
    private LocationData moveHelper(LocationData currentLoc,
                                    char marker)
    {
    ...
    /**
     * Moves maze mover from center of cell to edge of cell at maze exit
     * 
     * @param currentLoc LocationData object holding the maze mover's 
     * current location status
     * 
     ...
     * 
     * @param marker character marker to be placed at location in array
     */
    ...
    /**
     * Method sets back track marker to BACKTRACK_MARKER only if back track
     * set flag is true;
    ...
    /**
     * Method allows user to set the diagnostic display to show path
     * of maze mover as well as setting
     * the time in seconds for how long each display is paused
     * 
     * @param runFlag Boolean value that supports setting diagnostic run flag
     * 
     * @param displayTime double value in seconds for how long to
     * show display when diagnostic display is called
     */
    ...
    /**
     * Method moves maze mover to center of cell from edge of maze
     * where start location was given
     * 
     * @param currentLoc LocationData object holding the maze
     * mover status at starting edge of maze
     * 
     ...
     * <p>
     * Note: Calls solveMazeHelper with first cell position of maze mover
     * 
     * @return Boolean result of attempt to find path through maze
     */
    public boolean solveMaze()
    {
    	//solve maze used to set up out current location in the maze 
    	//as well as used to update that location.
    	getInitialDirection();
    	// CurLoc is my current location in the maze
    	LocationData CurLoc  = new LocationData( getInitialDirection() ,
    			getStartY(), getStartX() );
    	// The updated current location in the maze
    	LocationData UpdateCurLoc  = new LocationData( getInitialDirection() ,
    			getStartY(), getStartX() );
    	
    	CurLoc = setFirstPosition( CurLoc, TRACK_MARKER );
    	
    	// prints out Failed or Success depending on the maze Solution
    	if( !solveMazeHelper( CurLoc ) ) 
    	{
    		System.out.println( "Failed: Solution Not Found!" );
    		
    		UpdateCurLoc = setFirstPosition( UpdateCurLoc, BACKTRACK_MARKER );
    ...
    /**
     * Helper method for solveMaze, recursively called with
     * current maze mover location status
     * 
    ...
    {
    	// my working location
    	LocationData workinLocation  = new LocationData( currentLoc );
    	
    	// states that if there is an exit ahead we move to that location
    	if( isExitAhead( workinLocation ) ) 
    	{
    		moveToExit( workinLocation );
    		
    		return true;
    		
    	}
    	
    	// if there is no wall then we turn left
    	if( !isWallAhead( workinLocation ) ) 
    	{
    		workinLocation = moveToNextCell( workinLocation );
    	}
    	diagnosticDump();
    	
    	workinLocation = turnToLeft( workinLocation );
    	
    	//if there a wall then we turn right to look straight
    	if( !isWallAhead( workinLocation ) ) 
    	{
    		if( solveMazeHelper( workinLocation )) 
    		{
    			return true;
    	...
    	workinLocation = turnToRight(workinLocation);
    	
    	//if there is still a wall then we turn right again to look right
    	if( !isWallAhead( workinLocation ) ) 
    	{
    		if( solveMazeHelper( workinLocation )) 
    		{
    	...
    	// Finally if there is still a wall and we have checked all possible 
    	// direction then we recursively backtrack
    	if( !isWallAhead( workinLocation ) ) 
    	{
    		if( solveMazeHelper( workinLocation )) 
    		{
    	...
    	workinLocation = undoMoveToCell( currentLoc );
    	
    	diagnosticDump();
        
    	// we return false if there is no solution
        return false;
     ...
     * <p>
     * Note: Calls moverHelper to conduct this action; also passes in
     * currently set back track marker
     * 
     * @param currentLoc - LocationData object holding maze mover's
     * current location status
     * 
